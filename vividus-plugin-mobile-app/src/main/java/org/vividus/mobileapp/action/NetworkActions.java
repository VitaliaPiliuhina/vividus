...
public class NetworkActions
{
    ...
    public enum State
    {
        ON
                {
                    ...
                    {
                        if (genericWebDriverManager.isAndroid() && !webDriverProvider.getUnwrapped(AndroidDriver.class)
                                .getConnection().isWiFiEnabled())
                        {
        ...
        public void switchWiFiForIOS(IWebDriverProvider webDriverProvider, boolean isNeedWiFiOn)
        {
            ...
            if ((switchStatus.equalsIgnoreCase("1") && isNeedWiFiOn) || switchStatus.equalsIgnoreCase(
                    "0") && !isNeedWiFiOn)
            {
                switchBtn.click();
            }
