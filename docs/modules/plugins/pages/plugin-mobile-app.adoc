...
=== Enable network condition

Enable network state for a device.
...
|`WIFI`
|Enable Wi-Fi connection state only.

|`DATA`
|Enable data connection state only.

|`AIRPLANE`
|Enable Airplane Mode state only.

|`ALL`
|Enable data and WiFi state.
...
[source,gherkin]
----
When I enable `WIFI` state
----

=== Disable network condition

Disable network state for a device.
...
[source,gherkin]
----
When I disable `WIFI` state
----

=== Turn On/Off Wi-Fi

On/Off Wi-Fi for a device
...
[source,gherkin]
----
When I turn $mode Wi-Fi
----

* `$mode` - Name of the action to be performed
