...
Enable or Disable Wi-Fi and Mobile Network for device
...
[source,gherkin]
----
Scenario: Disable Wi-fi and Mobile Network. Enable Wi-fi and Mobile Network.
When I turn `OFF` the Network Connection
When I turn `ON` the Network Connection
----
...
Enable or Disable Wi-Fi for a device

TIP: For IOS devices: to return to the application  you must use a step:
When I activate application with bundle identifier `$bundleId`.
For Android devices: no need to return to the application because it remains active.
...
----
Scenario:
For IOS: Turn OFF Wi-Fi and activate an application using bundleId.

When I turn 'OFF` Wi-Fi
When I activate application with bundle identifier `$bundleId`

For Android: Turn OFF Wi-Fi. Turn ON Wi-Fi.
